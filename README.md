...
It uses free-tier cloud LLM providers (Groq, Cerebras, and optionally Mistral and OpenRouter) as its primary brain, rotating across providers and models each loop. A local Ollama model serves as survival-only fallback when all cloud providers are unavailable.
...
6. Set up cloud providers interactively (Groq and Cerebras required; Mistral and OpenRouter optional) with live validation
